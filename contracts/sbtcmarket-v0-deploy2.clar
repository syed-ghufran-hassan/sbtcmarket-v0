;; sBTC Prediction Market - Complete Set AMM (Phase 1)
...
(define-private (vault-deposit (amount uint))
  (contract-call? 'ST1F7QA2MDF17S807EPA36TSS8AMEFY4KA9TVGWXT.sbtc-token transfer amount tx-sender (get-contract-principal) none)
)
...
(define-private (vault-withdraw (amount uint) (recipient principal))
  (let ((contract-balance (unwrap! (contract-call? 'ST1F7QA2MDF17S807EPA36TSS8AMEFY4KA9TVGWXT.sbtc-token get-balance (get-contract-principal)) (err ERR-INSUFFICIENT-BALANCE))))
    ;; Ensure contract has enough sBTC to fulfill withdrawal
    (try! (ensure (>= contract-balance amount) ERR-INSUFFICIENT-BALANCE))
    (as-contract (contract-call? 'ST1F7QA2MDF17S807EPA36TSS8AMEFY4KA9TVGWXT.sbtc-token transfer amount (get-contract-principal) recipient none))
  )
)
...
(define-public (resolve-market
  ...
  (let ((market (try! (require-market-open market-id))))
    ...
    (try! (contract-call? 'STR738QQX1PVTM6WTDF833Z18T8R0ZB791TCNEFM.pyth-oracle-v4 verify-and-update-price-feeds vaa execution-plan))

    ;; Read price from Pyth storage
    (let ((price-response (try! (contract-call? 'STR738QQX1PVTM6WTDF833Z18T8R0ZB791TCNEFM.pyth-oracle-v4 get-price (get threshold-feed-id market)
...
(define-public (buy-shares
  ...
  (let (
      ...
      (tax-transfer (try! (contract-call? 'ST1F7QA2MDF17S807EPA36TSS8AMEFY4KA9TVGWXT.sbtc-token transfer tax-amount tx-sender (var-get tax-recipient) none)))
